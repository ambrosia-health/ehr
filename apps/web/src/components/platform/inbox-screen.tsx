...
const queue = [
  ...
  { id: "jordan", initials: "JL", patient: "Jordan Lee", episode: "Mohs closure", preview: "Wound check request remains unread.", stop: "Delivery fallback choice", due: "2h", status: "waiting" as const, owner: "Staff" },
  { id: "benjamin", initials: "BC", patient: "Benjamin Carter", episode: "Patient balance", preview: "$65 reminder paused.", stop: "Financial email consent expired", due: "Today", status: "waiting" as const, owner: "RCM" },
];
...
const threads: Record<string, { messages: { actor: string; body: string; time: string; kind: "patient" | "team" | "activity" }[]; draft: string; why: string; release: string }> = {
  ...
  jordan: { messages: [{ actor: "Care team", body: "Please upload a clear photo of the wound in good light.", time: "Yesterday", kind: "team" }, { actor: "Ambrosia", body: "Portal message delivered but unread for 48 hours. SMS is disabled by preference.", time: "8:05 AM", kind: "activity" }], draft: "Hi Jordan, we’re checking that you received your wound-photo request. Please call us if the portal is difficult to access.", why: "The approved Mohs closure program allows staff-approved phone outreach when secure messages remain unread for 48 hours.", release: "Create phone outreach for staff and defer the wound-check deadline by one business day." },
...
export function InboxScreen() {
  ...
  return <ScreenFrame>
    <ScreenHeader title="284 conversations are moving." description={`${queue.length - sent.size} need a person. Ambrosia is advancing 246 routine conversations under approved communication policies.`} action={<div className="flex flex-wrap gap-2"><StatusPill status="human">3 clinician</StatusPill><StatusPill status="waiting">6 staff</StatusPill><Button variant="outline">Programs <ChevronDown className="size-4" /></Button></div>} />
    <section className="border-b border-[#dce3db] px-5 py-4 sm:px-8 lg:px-10"><div className="mx-auto grid max-w-[1480px] grid-cols-2 gap-3 sm:grid-cols-5">{[["3", "Needs clinician", "human"], ["6", "Needs staff", "human"], ["2", "Delivery issue", "risk"], ["27", "Waiting on patient", "waiting"], ["246", "Advancing", "complete"]].map(([count, label, status]) => <button type="button" key={label} className="rounded-lg border border-[#d9dfd8] bg-white p-3 text-left hover:border-[#bfcbbf]"><p className="font-mono text-xl font-semibold">{count}</p><p className="mt-1 text-[10px] text-[#687870]">{label}</p><span className="sr-only">{status}</span></button>)}</div></section>
    ...
    <div className="mx-auto grid max-w-[1480px] xl:grid-cols-[310px_minmax(0,1fr)_330px]">
      ...
      <aside className="p-5 xl:min-h-[720px]"><SectionTitle title="Patient-agent context" description="The minimum clinical, financial, consent, and monitoring context for this conversation." />
        <div className="mt-5 space-y-5">
          ...
          <section className="border-t border-[#e0e5df] pt-4"><p className="text-[9px] font-semibold uppercase tracking-[0.14em] text-[#75827b]">Consent & channels</p><div className="mt-3 space-y-2">{[[MessageSquareText, "Secure portal", "Active"], [Smartphone, "SMS notifications", selected.id === "jordan" ? "Disabled" : "Verified"], [Phone, "Phone outreach", "Staff only"]].map(([Icon, label, value]) => { const I = Icon as typeof Phone; return <div key={String(label)} className="flex items-center gap-2 text-[10px]"><I className="size-3.5 text-[#2b654b]" /><span className="flex-1">{label as string}</span><span className="font-semibold">{value as string}</span></div>; })}</div></section>
